"""Artifact-first summaries for seed-oriented workbench runs."""
...
def persist_case_seed_summary(
    ...
    *,
    ...
    payload = build_case_seed_summary(case, stop_after=stop_after, preflight=preflight)
    json_path = repo.dump_output_json(case.case_id, "seed/case_summary.json", payload)
    markdown_path = repo.dump_output_text(
        case.case_id,
        "seed/case_summary.md",
        render_case_seed_summary(payload),
    )
    case.outputs["seed_case_summary_json"] = json_path
    case.outputs["seed_case_summary_markdown"] = markdown_path
    repo.save_case(case)
...
def persist_sweep_summary(
    ...
    *,
    ...
    run_dir = storage_dir / "seed_runs" / label
...
def render_case_seed_summary(summary: dict[str, Any]) -> str:
    lines = [
        f"# {summary['name']}",
        "",
...
def _important_outputs(case: CaseRecord) -> dict[str, str]:
    keys = [
        ...
        "codex_runner_log",
        "seed_orchestration_log",
        "seed_case_summary_json",
        "seed_case_summary_markdown",
    ]
